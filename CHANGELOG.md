...
- **List widget and ListProvider registry** — a new generic list widget that renders a scrollable, tappable list of items inside a pad button. Data is supplied by pluggable providers registered at runtime via `list_provider_register()`. Tapping an item dispatches a user-configured action template with `{id}` substitution (same pattern as the numeric rocker's `{step}`). The widget shows "No items" for empty providers and "Source not found" when the configured provider ID doesn't exist. The portal includes a List option in the widget dropdown, a select-action editor with `{id}` help text, grid preview indicator, and binding validator bypass (the data binding is a provider ID, not an MQTT topic). Covered by 14 host-native unit tests.
