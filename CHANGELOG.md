...
- **Minimum brightness floor** — display brightness is now clamped to a minimum of 5% (`MIN_USER_BRIGHTNESS`) across all adjustment paths: button actions, REST API, and NVS config load. Prevents the display from being set to zero brightness (effectively off) without using the screensaver. The floor is board-overridable via `#ifndef` guard.
