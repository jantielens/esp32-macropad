...
- **Configurable audio cues** — device-level tap and long-press beep patterns that play automatically when a button with an action is pressed. Configured in the Home page Audio section using the beep pattern DSL (e.g. `800:80` for a quick tap click, `600:40 40 600:40` for a double-chirp on long-press). Audio cues are only triggered when the button has a corresponding action configured — buttons with no action stay silent. Per-button overrides in the button editor's Audio Feedback section let you customize or suppress (enter `none`) the sound for individual buttons. Swipe gestures also use the device-level tap beep. Beep actions are auto-suppressed to avoid double-beep.
