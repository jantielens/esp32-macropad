...
// ============================================================================
// When called from meter_loop() (Arduino loop task), the caller holds
// g_meter_lock so the LVGL-task resolver never sees partially-updated
// state.  When called from LVGL-task commands (cmd_set_*, cmd_add_*),
// no lock is needed — the higher-priority LVGL task cannot be preempted
// by the loop task (both tasks run on Core 1).
//
// Must not contain logging or I/O — called under spinlock from meter_loop.

static void recompute() {
    // Need both bright and dark to compute anything
    if (s.l_bright < 0.0f || s.l_dark < 0.0f) {
        s.has_results = false;
        s.time_s = -1.0f;
        return;
    }
...
static bool meter_resolve(const char* params, char* out, size_t out_len) {
    ...
    if (strcmp(params, "lref") == 0) {
        ...
        snprintf(out, out_len, "%.4f", s.lref);
        return true;
    }
...
void meter_loop() {
    ...
    // Write result and recompute under spinlock so the LVGL-task
    // resolver never sees partially-updated state.  recompute()
    // does only fast FP math (log10f, powf, table scan) — no I/O,
    // no logging — so the critical section is <10µs.
    portENTER_CRITICAL(&g_meter_lock);
    if (pr == PENDING_LREF)   s.lref = lux;
    if (pr == PENDING_BRIGHT) s.l_bright = lux;
    if (pr == PENDING_DARK)   s.l_dark = lux;
    recompute();
    portEXIT_CRITICAL(&g_meter_lock);

    // Log and shared-mem update outside the critical section
    if (pr == PENDING_LREF) {
        shared_mem_set("lref", lux);
        LOGI(TAG, "Lref = %.4f lux", lux);
    } else if (pr == PENDING_BRIGHT) {
...
void meter_init() {
    // Auto-populate Lref from shared memory if previously set
    bool is_set = false;
    float val = shared_mem_get("lref", &is_set);
    if (is_set && val > 0.0f) {
        s.lref = val;
        LOGI(TAG, "Lref auto-populated from shared memory: %.4f", val);
    }
