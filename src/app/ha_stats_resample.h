...
// `source_period_ms` is the Recorder period (five minutes or one hour). Values
// from consecutive periods are linearly interpolated across finer stream
// slots; missing periods retain the previous value. Slots before the first
// known Recorder value remain NAN. When several periods land in one bucket the
// last one wins, matching the live ingest path (which also overwrites within a
// slot rather than averaging).
//
