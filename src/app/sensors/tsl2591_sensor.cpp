...
// When SENSOR_I2C_BUS == 1 the TSL2591 sits on a dedicated Wire1 bus
...
bool tsl2591_init() {
    ...
    SENSOR_WIRE.begin(SENSOR_I2C_SDA, SENSOR_I2C_SCL);
    SENSOR_WIRE.setClock(SENSOR_I2C_FREQUENCY);
    LOGI(TAG, "Wire1 started on SDA=%d SCL=%d", SENSOR_I2C_SDA, SENSOR_I2C_SCL);
#endif
...
void register_tsl2591_sensor(SensorRegistry &registry) {
    ...
    callbacks.name = "TSL2591";
    // init/read are demand-driven — no polling callbacks needed.
...
void  register_tsl2591_sensor(SensorRegistry &) {}
