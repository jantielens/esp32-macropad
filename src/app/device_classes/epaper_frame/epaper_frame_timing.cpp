...
struct EpaperWakeJournal {
		...
		EpaperWakeRecord records[EPAPER_FRAME_WAKE_JOURNAL_CAPACITY];
};
...
static EpaperWakeRecord* current_wake_record() {
		...
		const uint8_t index = (uint8_t)((s_epaper_wake_journal.head +
				s_epaper_wake_journal.count - 1) % EPAPER_FRAME_WAKE_JOURNAL_CAPACITY);
...
uint32_t epaper_frame_timing_begin_wake(EpaperWakeReason wake_reason) {
		...
		if (s_epaper_wake_journal.count == EPAPER_FRAME_WAKE_JOURNAL_CAPACITY) {
			s_epaper_wake_journal.head = (uint8_t)((s_epaper_wake_journal.head + 1) %
					EPAPER_FRAME_WAKE_JOURNAL_CAPACITY);
			--s_epaper_wake_journal.count;
			++s_epaper_wake_journal.dropped_count;
		}
		const uint8_t index = (uint8_t)((s_epaper_wake_journal.head +
				s_epaper_wake_journal.count) % EPAPER_FRAME_WAKE_JOURNAL_CAPACITY);
		EpaperWakeRecord& record = s_epaper_wake_journal.records[index];
		record = {};
		record.timing = epaper_frame_timing_last;
...
void epaper_frame_wake_journal_complete_delivery(uint32_t mqtt_connect_ms,
		...
		ensure_wake_journal();
		s_epaper_previous_delivery.session_id = epaper_frame_timing_last.session_id;
		s_epaper_previous_delivery.wake_id = epaper_frame_timing_last.wake_id;
...
void epaper_frame_timing_set_resolve_ms(uint32_t ms) {
		epaper_frame_timing_last.resolve_ms = ms;
}
