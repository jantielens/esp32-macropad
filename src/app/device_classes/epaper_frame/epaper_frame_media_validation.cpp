#include "epaper_frame_media_validation.h"
...
EpaperMediaValidation epaper_frame_validate_g16p(const uint8_t* data, size_t len,
		...
		if (epaper_frame_transport_crc32(data + kG16pHeaderSize, expected_payload) != payload_crc) {
				return EpaperMediaValidation::PayloadCrcMismatch;
		}
		return EpaperMediaValidation::Valid;
}

EpaperMediaValidation epaper_frame_validate_jpeg(const uint8_t* data, size_t len,
		uint16_t panel_width, uint16_t panel_height) {
...
EpaperMediaValidation epaper_frame_validate_g16z_completion(int status, int done_status,
