...
#include <stddef.h>
#ifndef EPAPER_FRAME_DRIVER_H
#define EPAPER_FRAME_DRIVER_H
...
// aggregated by epaper_frame_drivers.cpp.
//
// Lifecycle on each wake (duty_cycle_epaper_frame):
//   1. epaper_frame_driver_begin()           — power up the panel
//   2. epaper_frame_driver_set_rotation(r)
//   3. epaper_frame_driver_draw_url(url)     — fetch over HTTP(S) + decode + draw
//   4. epaper_frame_driver_display()         — push framebuffer; false if not confirmed
//   5. epaper_frame_driver_sleep()           — power down the panel before deep sleep
//
...
// ----------------------------------------------------------------------------
// epaper_frame_driver_begin_async() kicks off epaper_frame_driver_begin() so the caller can
// run independent work (e.g. the WiFi association) concurrently with the slow
// panel power-up. epaper_frame_driver_begin_join() blocks until init has finished and
...
//
// epaper_frame_driver_battery_ready_before_begin() reports whether the board can read
...
bool epaper_frame_driver_battery_ready_before_begin();

// ----------------------------------------------------------------------------
// Optional SD image cache (boards with a shared-bus microSD slot only).
// When enabled, epaper_frame_driver_draw_url() serves the image straight from SD on
...
// without an SD cache.
//
// On boards that define EPAPER_FRAME_SD_CS_PIN the driver implements these as thin
// pass-throughs to the shared epaper_frame_sd_cache module. On boards without it,
// they resolve to inline no-ops provided by epaper_frame_sd_cache.h (included below)
...
// epaper_frame_driver_display() so the ~1-2 s write lands in the awake tail rather
...
// three vtable functions above on boards without EPAPER_FRAME_SD_CS_PIN.
...
#define EPAPER_FRAME_BLACK      0
#define EPAPER_FRAME_DARK_GRAY  2
#define EPAPER_FRAME_LIGHT_GRAY 5
#define EPAPER_FRAME_WHITE      7
...
#define EPAPER_FRAME_FONT_SMALL  0
#define EPAPER_FRAME_FONT_MEDIUM 1
#define EPAPER_FRAME_FONT_LARGE  2
...
void epaper_frame_driver_set_font(uint8_t font_id);         // EPAPER_FRAME_FONT_SMALL/MEDIUM/LARGE
...
void  epaper_frame_driver_show_vcom_test_pattern(float preview_vcom = NAN);
...
void epaper_frame_driver_frontlight_on(uint8_t brightness);  // 0..63; 0 is no-op
void epaper_frame_driver_frontlight_off();
#endif

#endif // IS_EPAPER_FRAME

#endif // EPAPER_FRAME_DRIVER_H
