...
EpaperNextAction epaper_frame_next_action_for_status(int status) {
		switch (status) {
...
bool epaper_frame_source_advances_carousel(EpaperSourceMode mode, uint8_t carousel_count) {
		return !epaper_frame_source_uses_service(mode) && carousel_count > 0;
}

uint32_t epaper_frame_source_refresh_interval(EpaperSourceMode mode,
		uint32_t service_interval_seconds, uint32_t slot_interval_seconds) {
		return epaper_frame_source_uses_service(mode)
				? service_interval_seconds : slot_interval_seconds;
}

bool epaper_frame_next_use_cached_blob(bool cache_enabled, bool cache_valid) {
		return cache_enabled && cache_valid;
}

bool epaper_frame_next_parse_crc32(const char* value, uint32_t* out_crc) {
...
bool epaper_frame_next_valid_image_key(const char* value) {
