...
// invokes one of the epaper_frame_screen_* functions below (or composites several).
// Templated so callers can pass a lambda that captures local state without
// std::function overhead.
template <typename Fn>
inline void epaper_frame_show_status(uint8_t rotation, Fn paint) {
	if (epaper_frame_driver_begin()) {
		epaper_frame_driver_set_rotation(rotation);
		paint();
...
// NOT call epaper_frame_driver_display() — the caller decides when to push the
...
// their window.
//
// Callers are also responsible for epaper_frame_driver_begin() / epaper_frame_driver_sleep().
// ----------------------------------------------------------------------------
...
// EPAPER_FRAME_FAST_REFRESH. Skipped on slow panels where the extra full refresh
...
// be up first. The full epaper_frame_screen_config_mode() screen is still drawn
