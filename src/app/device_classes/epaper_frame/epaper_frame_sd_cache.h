#pragma once
#ifndef EPAPER_FRAME_SD_CACHE_H
#define EPAPER_FRAME_SD_CACHE_H
...
//
// On a cache hit, epaper_frame_driver_draw_url() reads the blob straight from SD and
...
//
// Compiled in only when the board defines EPAPER_FRAME_SD_CS_PIN. On boards without
// it, the SD-cache HAL vtable (epaper_frame_driver.h) resolves to the inline no-ops
// below — the single source of truth, so per-driver stubs are not needed.
// ----------------------------------------------------------------------------

#if defined(EPAPER_FRAME_SD_CS_PIN)
...
void epaper_frame_sd_cache_init(const EpaperSdCacheConfig& cfg);

// Enable/disable SD caching for subsequent draws. Call before draw_url().
void epaper_frame_sd_cache_set_enabled(bool enabled);

// Whether SD caching is currently enabled (runtime toggle).
bool epaper_frame_sd_cache_is_enabled();
...
bool epaper_frame_sd_cache_read(uint32_t content_crc32, uint8_t** out_buf, size_t* out_len);
...
// Write the staged blob to SD (call after epaper_frame_driver_display()). Always
// clears the staging slot, whether or not the write succeeded. No-op when
// nothing is staged.
void epaper_frame_sd_cache_flush();

// Discard the staged blob without writing it (frees the pending buffer).
void epaper_frame_sd_cache_discard_pending();

// Wipe the on-SD cache (portal "Clear SD cache" action). Returns true when the
// cache was cleared (or was already empty).
bool epaper_frame_sd_cache_clear();

#else // !defined(EPAPER_FRAME_SD_CS_PIN)

// Boards without a shared-bus microSD slot: the SD-cache vtable declared in
// epaper_frame_driver.h resolves to these inline no-ops. This is the single source of
// truth for the "no SD cache on this board" behaviour — individual drivers do
// not (and must not) define their own stubs.
inline void epaper_frame_driver_set_sd_cache_enabled(bool /*enabled*/) {}
inline void epaper_frame_driver_cache_flush() {}
inline void epaper_frame_sd_cache_discard_pending() {}
inline bool epaper_frame_driver_sd_cache_clear() { return false; }

#endif // defined(EPAPER_FRAME_SD_CS_PIN)

#endif // IS_EPAPER_FRAME

#endif // EPAPER_FRAME_SD_CACHE_H
