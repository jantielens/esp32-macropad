...
bool parse_show_metadata(HTTPClient& http, ShowMetadata* metadata) {
		...
		if (!epaper_frame_next_valid_image_key(image_key.c_str()) ||
				!epaper_frame_next_parse_crc32(crc_text.c_str(), &metadata->content_crc32)) {
				return false;
		}
...
bool try_cache(uint32_t content_crc32, const char* media_type,
		...
		size_t len = 0;
		if (!epaper_frame_sd_cache_read(content_crc32, &data, &len)) return false;
		const bool valid = epaper_frame_transport_crc32(data, len) == content_crc32 &&
				epaper_frame_driver_prepare_service_blob(data, len, media_type,
...
bool read_and_validate_body(HTTPClient& http, const ShowMetadata& metadata,
		...
		if (!epaper_frame_http_read_body(http, &data, &len, &body_bytes_read,
				false, kServiceHttpTimeoutMs)) return false;
		payload->body_bytes_read = body_bytes_read;
		if (epaper_frame_transport_crc32(data, len) != metadata.content_crc32 ||
				!epaper_frame_driver_prepare_service_blob(data, len, media_type,
						&payload->prepared_data, &payload->prepared_len)) {
			heap_caps_free(data);
			return false;
...
EpaperNextPayload epaper_frame_next_client_fetch(const char* service_base,
		...
		for (uint8_t attempt = 0; attempt < max_cycles; ++attempt) {
				...
				const int status = http.GET();
				const EpaperNextAction action = epaper_frame_next_action_for_status(status);
