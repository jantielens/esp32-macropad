#ifndef EPAPER_FRAME_CONFIG_H
#define EPAPER_FRAME_CONFIG_H
...
#include "epaper_frame_next_client_logic.h"
...
struct EpaperConfig {
		...
		char epaper_frame_url[CONFIG_EPAPER_URL_MAX_LEN];   // resolved runtime URL for current carousel slot (not user-configured)
		...
		bool epaper_frame_sd_cache_enabled;                 // cache downloaded image blobs (G16Z/G16P) on microSD to skip the HTTP download on a cache hit (default false; only effective on boards with EPAPER_FRAME_SD_CS_PIN)

		// On-image status overlay
		bool epaper_frame_overlay_enabled;                  // default false
		uint8_t epaper_frame_overlay_position;              // 0=TL, 1=TR, 2=BL, 3=BR (default 3)
		uint8_t epaper_frame_overlay_color;                 // 0=black, 1=darkgray, 2=lightgray, 3=white (default 0)
		uint8_t epaper_frame_overlay_items;                 // bitmask: 0x1=batt icon, 0x2=batt %, 0x4=time, 0x8=cycle ms
		...
		uint8_t epaper_frame_frontlight_brightness;         // 0..63 (0 = disabled, default 0)
		uint16_t epaper_frame_frontlight_duration_s;        // seconds after button wake (default 30)
...
// Single global instance, owned by epaper_frame_device_class.cpp. Other e-paper
// modules read directly from this.
extern EpaperConfig g_epaper_config;

// Resolve g_epaper_config.epaper_frame_url from the current carousel slot.
