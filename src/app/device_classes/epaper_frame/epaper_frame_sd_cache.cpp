// E-paper SD blob cache implementation. See epaper_frame_sd_cache.h for the contract.
//
// Aggregated into the build by device_classes/epaper_frame_device_class.cpp (arduino-
// cli only compiles .cpp files in the sketch root). The entire body is
// #if-stripped on boards that do not define EPAPER_FRAME_SD_CS_PIN, so it links out
// cleanly and the HAL vtable falls back to the header inline no-ops.

#include "board_config.h"

#if IS_EPAPER_FRAME && defined(EPAPER_FRAME_SD_CS_PIN)
...
// Hardware wiring captured by epaper_frame_sd_cache_init().
...
#endif // IS_EPAPER_FRAME && defined(EPAPER_FRAME_SD_CS_PIN)
