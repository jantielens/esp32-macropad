...
uint8_t resolve_color(uint8_t color_id) {
		switch (color_id) {
				case 0: return EPAPER_FRAME_BLACK;
				case 1: return EPAPER_FRAME_DARK_GRAY;
				case 2: return EPAPER_FRAME_LIGHT_GRAY;
				case 3: return EPAPER_FRAME_WHITE;
				default: return EPAPER_FRAME_BLACK;
		}
}
...
void draw_battery_icon(int16_t x, int16_t y, uint8_t pct, uint8_t color, uint8_t bg) {
		...
		// Body
		epaper_frame_driver_fill_rect(x, y, bw, bh, bg);
		epaper_frame_driver_draw_rect(x, y, bw, bh, color);
		// Cap
		epaper_frame_driver_fill_rect(x + bw, y + (bh - cap_h) / 2, cap_w, cap_h, color);
		// Fill
		int16_t fill_w = ((int)(bw - 4) * pct + 50) / 100;
		if (fill_w < 0) fill_w = 0;
		if (fill_w > bw - 4) fill_w = bw - 4;
		if (fill_w > 0) {
				epaper_frame_driver_fill_rect(x + 2, y + 2, fill_w, bh - 4, color);
		}
}
...
void epaper_frame_overlay_render(uint16_t battery_mv, uint32_t cycle_time_ms) {
		if (!g_epaper_config.epaper_frame_overlay_enabled) return;
		if (g_epaper_config.epaper_frame_overlay_items == 0) return;
		...
		const uint8_t pct = epaper_frame_battery_percent(battery_mv);
		char chunk[24];

		if (g_epaper_config.epaper_frame_overlay_items & kItemBattPct) {
		...
		if (g_epaper_config.epaper_frame_overlay_items & kItemCycleTime) {
				snprintf(chunk, sizeof(chunk), "%s%ums", len ? "  " : "", (unsigned)cycle_time_ms);
				append(chunk);
		}

		const bool show_icon = (g_epaper_config.epaper_frame_overlay_items & kItemBattIcon) != 0;
		const bool show_text = (len > 0);
		if (!show_icon && !show_text) return;

		epaper_frame_driver_set_font(EPAPER_FRAME_FONT_MEDIUM);
		const uint8_t color = resolve_color(g_epaper_config.epaper_frame_overlay_color);
		...
		const uint8_t bg = (color == EPAPER_FRAME_BLACK || color == EPAPER_FRAME_DARK_GRAY)
				? EPAPER_FRAME_WHITE : EPAPER_FRAME_BLACK;

		int16_t tx1 = 0, ty1 = 0; uint16_t tw = 0, th = 0;
		if (show_text) {
				epaper_frame_driver_get_text_bounds(text, 0, 0, &tx1, &ty1, &tw, &th);
		}
		...
		int16_t bx = 0, by = 0;
		switch (g_epaper_config.epaper_frame_overlay_position) {
		...
		epaper_frame_driver_fill_round_rect(bx, by, box_w, box_h, 8, bg);
		...
		if (show_text) {
				...
				// is content_top - ty1.
				epaper_frame_driver_set_text_color(color);
				epaper_frame_driver_set_cursor(cx - tx1, cy - ty1);
