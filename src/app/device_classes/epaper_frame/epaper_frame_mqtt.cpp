...
bool epaper_frame_mqtt_publish_state(const EpaperRefreshOutcome& outcome,
		...
		doc["refresh_count"]   = epaper_frame_refresh_get_count();
		doc["sidecar_http_status"] = outcome.sidecar_http_status;
		doc["source_mode"] = epaper_frame_source_uses_service(g_epaper_config.source_mode)
...
bool epaper_frame_mqtt_publish_pending_wakes(uint32_t reporting_wake_id) {
		...
		while (epaper_frame_wake_journal_peek(&record)) {
			...
			const uint32_t dropped_wake_records = epaper_frame_wake_journal_dropped_count();
			...
			if (dropped_wake_records > 0) epaper_frame_wake_journal_clear_dropped_count();
...
bool epaper_frame_mqtt_publish_ha_discovery(MqttManager& mqtt) {
	// Publishes retained HA discovery configs for the e-paper
		...
		const char* legacy_timing_entities[] = {
				"epaper_frame_boot_to_wifi_ms", "epaper_frame_ntp_sync_ms", "epaper_frame_crc_to_draw_ms",
				"epaper_frame_draw_to_mqtt_ms", "epaper_frame_last_elapsed_ms", "epaper_frame_crc_retries",
				"epaper_frame_resolve_ms", "epaper_frame_fetch_ms", "epaper_frame_draw_ms", "epaper_frame_image_source",
		};
		...
		publish_sensor("epaper_frame_battery", "Battery",
									 "{{ value_json.battery_pct }}", "%", "battery", "measurement");
		publish_sensor("epaper_frame_battery_mv", "Battery Voltage",
		...
		delay(1);
		publish_sensor("epaper_frame_image_crc", "E-Paper Image CRC",
									 "{{ '0x%08x' | format(value_json.image_crc32) }}", "", "", "");
		publish_sensor("epaper_frame_sidecar_http", "E-Paper Sidecar HTTP Status",
									 "{{ value_json.sidecar_http_status }}", "", "", "measurement");
		delay(1);

		publish_sensor("epaper_frame_loop_ms", "E-Paper Wake Loop Time",
