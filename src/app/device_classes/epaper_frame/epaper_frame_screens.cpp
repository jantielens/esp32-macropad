...
void draw_border() {
		const int16_t w = epaper_frame_driver_width();
		const int16_t h = epaper_frame_driver_height();
		epaper_frame_driver_draw_round_rect(BORDER_INSET, BORDER_INSET,
		                              w - BORDER_INSET * 2, h - BORDER_INSET * 2,
		                              24, EPAPER_FRAME_DARK_GRAY);
}

int16_t font_line_height(uint8_t font_id) {
		epaper_frame_driver_set_font(font_id);
		...
		epaper_frame_driver_get_text_bounds("Aygj", 0, 0, &x1, &y1, &w, &h);
		return (int16_t)h;
}

void draw_centered_baseline(const char* text, int16_t baseline_y) {
		int16_t x1, y1; uint16_t w, h;
		epaper_frame_driver_get_text_bounds(text, 0, baseline_y, &x1, &y1, &w, &h);
		const int16_t panel_w = epaper_frame_driver_width();
		const int16_t x = (panel_w - (int16_t)w) / 2 - x1;
		epaper_frame_driver_set_cursor(x, baseline_y);
		epaper_frame_driver_print(text);
}

class StatusScreenBuilder {
public:
		StatusScreenBuilder& heading1(const char* t) { return add(t, EPAPER_FRAME_FONT_LARGE,  EPAPER_FRAME_BLACK); }
		StatusScreenBuilder& heading2(const char* t) { return add(t, EPAPER_FRAME_FONT_MEDIUM, EPAPER_FRAME_BLACK); }
		StatusScreenBuilder& text(const char* t)     { return add(t, EPAPER_FRAME_FONT_MEDIUM, EPAPER_FRAME_BLACK); }
		StatusScreenBuilder& muted(const char* t)    { return add(t, EPAPER_FRAME_FONT_MEDIUM, EPAPER_FRAME_DARK_GRAY); }
		...
		void draw() const {
				...
				// previous line).
				const int16_t panel_h = epaper_frame_driver_height();
				int16_t baseline = (panel_h - total) / 2 + heights[0];
				for (uint8_t i = 0; i < count_; ++i) {
						epaper_frame_driver_set_font(lines_[i].font);
						epaper_frame_driver_set_text_color(lines_[i].color);
...
void epaper_frame_screen_boot_splash(const char* device_name, const char* version) {
		char ver[80];
...
void epaper_frame_screen_manual_refresh(const char* message) {
		StatusScreenBuilder()
				.heading1("Refreshing")
				.spacer()
...
void epaper_frame_screen_config_mode_starting() {
		StatusScreenBuilder()
...
void epaper_frame_screen_config_mode(const char* ssid, const char* ip, bool is_ap) {
		char url[80];
...
void epaper_frame_screen_returning_to_normal() {
		StatusScreenBuilder()
...
void epaper_frame_screen_error(const char* error_detail, uint32_t retry_seconds) {
...
void epaper_frame_screen_low_battery(uint16_t mv, uint8_t pct) {
		char level[64];
