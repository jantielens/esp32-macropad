...
// #include-aggregated into epaper_frame_device_class.cpp.
...
bool epaper_frame_http_read_body(HTTPClient& http, uint8_t** out_buf, size_t* out_len,
...
bool epaper_frame_http_download(const char* url, uint8_t** out_buf, size_t* out_len) {
	*out_buf = nullptr;
	*out_len = 0;
	...
	for (int hop = 0; hop <= kMaxRedirects; ++hop) {
		...
		const bool read_ok = epaper_frame_http_read_body(http, out_buf, out_len, &body_bytes_read);
