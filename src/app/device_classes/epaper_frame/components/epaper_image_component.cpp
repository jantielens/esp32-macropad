...
static void epaper_frame_image_result_json(AsyncWebServerRequest* request, const EpaperRefreshOutcome& out) {
...
static void epaper_frame_image_show_url_post(AsyncWebServerRequest* request) {
    ...
    EpaperRefreshOutcome out = epaper_frame_refresh_show_url(cfg, url.c_str());
    epaper_frame_image_result_json(request, out);
}
...
static void epaper_frame_image_clear_cache_post(AsyncWebServerRequest* request) {
    if (!portal_auth_gate(request)) return;
    LOGI("API", "POST /api/component/epaper-image/clear-sd-cache");
    const LoopBridgeResult result = loop_bridge_enqueue(epaper_frame_image_clear_cache_exec, nullptr, 0);
...
static ComponentDef epaper_frame_image_component = {
    ...
    .custom_actions = epaper_frame_image_actions,
    .num_custom_actions = sizeof(epaper_frame_image_actions) / sizeof(epaper_frame_image_actions[0]),
    .fragment_id = "epaper-image",
};

REGISTER_COMPONENT(epaper_frame_image);
