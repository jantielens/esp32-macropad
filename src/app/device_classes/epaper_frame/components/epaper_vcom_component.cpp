...
#if IS_EPAPER_FRAME && HAS_EPAPER_VCOM
...
static void epaper_frame_vcom_get(AsyncWebServerRequest* request) {
    ...
    const float v = epaper_frame_driver_read_vcom();
...
static void epaper_frame_vcom_post(AsyncWebServerRequest* request) {
    if (!portal_auth_gate(request)) return;
    ...
    const bool ok = epaper_frame_driver_write_vcom(v);
    StaticJsonDocument<128> resp;
    resp["success"] = ok;
    resp["vcom"] = v;
...
static void epaper_frame_vcom_test_pattern_post(AsyncWebServerRequest* request) {
    if (!portal_auth_gate(request)) return;
    ...
    // epaper_frame_driver_write_vcom(): -5.0 .. <0.0 V.
    float preview = NAN;
    ...
    epaper_frame_driver_show_vcom_test_pattern(preview);
    request->send(200, "application/json", "{\"success\":true}");
}

static const ComponentAction epaper_frame_vcom_actions[] = {
    {"vcom",              HTTP_GET,  epaper_frame_vcom_get,               nullptr},
    {"vcom",              HTTP_POST, epaper_frame_vcom_post,              nullptr},
    {"vcom-test-pattern", HTTP_POST, epaper_frame_vcom_test_pattern_post, nullptr},
};

static ComponentDef epaper_frame_vcom_component = {
    .id = "epaper-vcom",
    ...
    .custom_actions = epaper_frame_vcom_actions,
    .num_custom_actions = sizeof(epaper_frame_vcom_actions) / sizeof(epaper_frame_vcom_actions[0]),
    .fragment_id = "epaper-vcom",
};

REGISTER_COMPONENT(epaper_frame_vcom);

#endif // IS_EPAPER_FRAME && HAS_EPAPER_VCOM
