...
#include "device_classes/epaper_frame/epaper_frame_battery.h"
...
static void epaper_frame_status_refresh_post(AsyncWebServerRequest* request) {
    ...
    } else if (!epaper_frame_resolve_current_url()) {
        request->send(400, "application/json",
    ...
    EpaperRefreshOutcome out = epaper_frame_refresh_run(cfg, true /*force*/);
...
static void epaper_frame_status_get(AsyncWebServerRequest* request) {
    if (!portal_auth_gate(request)) return;

    const DeviceConfig* cfg = web_portal_get_current_config();
    const uint32_t last_unix = epaper_frame_refresh_last_unix();
    const EpaperRefreshOutcome last = epaper_frame_refresh_last_outcome();
    ...
    resp["refresh_count"]   = epaper_frame_refresh_get_count();
    resp["source_mode"] = epaper_frame_source_uses_service(g_epaper_config.source_mode)
    ...
    resp["battery_pct"]     = epaper_frame_battery_percent(last.battery_mv);
    resp["last_crc32"]      = g_epaper_config.epaper_frame_last_crc32;
    ...
    t["boot_to_wifi_ms"] = epaper_frame_timing_last.boot_to_wifi_ms;
    t["wifi_rssi"]       = epaper_frame_timing_last.wifi_rssi;
    t["ntp_sync_ms"]     = epaper_frame_timing_last.ntp_sync_ms;
    t["crc_to_draw_ms"]  = epaper_frame_timing_last.crc_to_draw_ms;
    t["draw_to_mqtt_ms"] = epaper_frame_timing_last.draw_to_mqtt_ms;
    t["total_active_ms"] = epaper_frame_timing_last.total_active_ms;
    ...
    hrdy["last_timeout_phase"] = epaper_frame_driver_hrdy_phase_name(
...
static const ComponentAction epaper_frame_status_actions[] = {
    {"refresh", HTTP_POST, epaper_frame_status_refresh_post, nullptr},
    {"status",  HTTP_GET,  epaper_frame_status_get,          nullptr},
};

static ComponentDef epaper_frame_status_component = {
    .id = "epaper-status",
    .category = "epaper_frame",
    .display_name = "Status",
    .nav_order = 10,
    .get_config = nullptr,
    .save_config = nullptr,
    ...
    .custom_actions = epaper_frame_status_actions,
    .num_custom_actions = sizeof(epaper_frame_status_actions) / sizeof(epaper_frame_status_actions[0]),
    .fragment_id = "epaper-status",
};

REGISTER_COMPONENT(epaper_frame_status);
