...
// board is one of these Inkplate targets and IS_EPAPER_FRAME is enabled. Wired into
// the build through device_classes/epaper_frame/epaper_frame_drivers.cpp.

#include "board_config.h"

#if IS_EPAPER_FRAME && (defined(BOARD_INKPLATE5V2_FRAME) || defined(BOARD_INKPLATE6FLICK_FRAME))
...
// instance lazily on first `epaper_frame_driver_begin()` call from `setup()`.
...
bool epaper_frame_driver_begin() {
		...
		s_began = true;
#if defined(BOARD_INKPLATE6FLICK_FRAME)
...
void epaper_frame_driver_begin_async() { s_begin_result = epaper_frame_driver_begin(); }
bool epaper_frame_driver_begin_join() { return s_begin_result; }
bool epaper_frame_driver_battery_ready_before_begin() { return false; }
...
bool epaper_frame_driver_draw_url(const char* url) {
		...
		// Download the image ourselves (the shared epaper_frame_http_download avoids the
		// InkplateLibrary HTTPS downloader, which crashes on https:// hosts — see
		// device_classes/epaper_frame/epaper_frame_http.h), then sniff the format from the magic
...
// SD-cache HAL vtable resolves to the inline no-ops in epaper_frame_sd_cache.h.
...
void epaper_frame_driver_show_vcom_test_pattern(float preview_vcom) {
		...
		if (preview_mode) {
				...
				displayed_vcom = epaper_frame_driver_read_vcom();
		}
		...
		// Centered label in EPAPER_FRAME_FONT_MEDIUM against a white strip so it's
		...
		s_display->setFont(s_font_table[EPAPER_FRAME_FONT_MEDIUM]);
		s_display->setTextColor(EPAPER_FRAME_BLACK);
...
void epaper_frame_driver_frontlight_on(uint8_t brightness) {
...
#endif // IS_EPAPER_FRAME && (BOARD_INKPLATE5V2_FRAME || BOARD_INKPLATE6FLICK_FRAME)
