...
// HSPI. Only compiled when the board is reTerminal E1003 and IS_EPAPER_FRAME is
// enabled. Wired into the build through device_classes/epaper_frame/epaper_frame_drivers.cpp.
//
...
#include "device_classes/epaper_frame/epaper_frame_media_validation.h"
...
#ifndef EPAPER_FRAME_FONT_SMALL_PTR
#define EPAPER_FRAME_FONT_SMALL_PTR  (&Inter_Regular8pt7b)
#endif
#ifndef EPAPER_FRAME_FONT_MEDIUM_PTR
#define EPAPER_FRAME_FONT_MEDIUM_PTR (&Inter_Regular12pt7b)
#endif
#ifndef EPAPER_FRAME_FONT_LARGE_PTR
#define EPAPER_FRAME_FONT_LARGE_PTR  (&Inter_Bold20pt7b)
#endif

// --- Panel geometry (native landscape) --------------------------------------
#ifndef EPAPER_FRAME_PANEL_W
#define EPAPER_FRAME_PANEL_W 1872
#endif
#ifndef EPAPER_FRAME_PANEL_H
#define EPAPER_FRAME_PANEL_H 1404
#endif

// --- IT8951 HSPI pin map (E1003 defaults; overridable per board) ------------
#ifndef EPAPER_FRAME_PIN_SCK
#define EPAPER_FRAME_PIN_SCK 7
#endif
#ifndef EPAPER_FRAME_PIN_MISO
#define EPAPER_FRAME_PIN_MISO 8
#endif
#ifndef EPAPER_FRAME_PIN_MOSI
#define EPAPER_FRAME_PIN_MOSI 9
#endif
#ifndef EPAPER_FRAME_PIN_CS
#define EPAPER_FRAME_PIN_CS 10
#endif
#ifndef EPAPER_FRAME_PIN_BUSY
#define EPAPER_FRAME_PIN_BUSY 13  // IT8951 HRDY
#endif
#ifndef EPAPER_FRAME_PIN_TFT_ENABLE
#define EPAPER_FRAME_PIN_TFT_ENABLE 11
#endif
#ifndef EPAPER_FRAME_PIN_ITE_ENABLE
#define EPAPER_FRAME_PIN_ITE_ENABLE 21
#endif
...
#ifndef EPAPER_FRAME_PIN_RES
#define EPAPER_FRAME_PIN_RES 12
#endif
...
#ifndef EPAPER_FRAME_VCOM_MV
#define EPAPER_FRAME_VCOM_MV 1400
#endif
...
#ifndef EPAPER_FRAME_BATTERY_ADC_PIN
#define EPAPER_FRAME_BATTERY_ADC_PIN -1
#endif
#ifndef EPAPER_FRAME_BATTERY_ENABLE_PIN
#define EPAPER_FRAME_BATTERY_ENABLE_PIN -1  // pin driven HIGH to gate the divider, if any
#endif
#ifndef EPAPER_FRAME_BATTERY_DIVIDER
#define EPAPER_FRAME_BATTERY_DIVIDER 2.0f  // typical 1:1 resistor divider
#endif
...
#ifndef EPAPER_FRAME_SPI_HZ
#define EPAPER_FRAME_SPI_HZ 14000000
#endif
...
constexpr size_t G16P_PAYLOAD_SIZE = (size_t)EPAPER_FRAME_PANEL_W * EPAPER_FRAME_PANEL_H / 2;
...
inline uint8_t hal_to_gray4(uint8_t c) {
		if (c >= EPAPER_FRAME_WHITE) return 15;
...
SPIClass s_spi(HSPI);
SPISettings s_spi_set(EPAPER_FRAME_SPI_HZ, MSBFIRST, SPI_MODE0);
bool s_began = false;
...
const GFXfont* const s_font_table[3] = {
		EPAPER_FRAME_FONT_SMALL_PTR,   // EPAPER_FRAME_FONT_SMALL
		EPAPER_FRAME_FONT_MEDIUM_PTR,  // EPAPER_FRAME_FONT_MEDIUM
		EPAPER_FRAME_FONT_LARGE_PTR,   // EPAPER_FRAME_FONT_LARGE
};
...
bool wait_hrdy(uint32_t timeout_ms = HRDY_COMMAND_TIMEOUT_MS) {
		...
		while (digitalRead(EPAPER_FRAME_PIN_BUSY) == LOW) {
...
bool write_cmd16(uint16_t cmd) {
		...
		digitalWrite(EPAPER_FRAME_PIN_CS, LOW);
		s_spi.transfer16(0x6000);
		if (!wait_hrdy()) {
				digitalWrite(EPAPER_FRAME_PIN_CS, HIGH);
...
bool read_dev_info() {
		...
		if (buf[0] != EPAPER_FRAME_PANEL_W || buf[1] != EPAPER_FRAME_PANEL_H ||
...
bool initialize_controller(bool power_cycle) {
		...
		pinMode(EPAPER_FRAME_SD_CS_PIN, OUTPUT);
		digitalWrite(EPAPER_FRAME_SD_CS_PIN, HIGH);
#endif
		s_spi.end();
		if (power_cycle) {
			digitalWrite(EPAPER_FRAME_PIN_ITE_ENABLE, LOW);
			digitalWrite(EPAPER_FRAME_PIN_TFT_ENABLE, LOW);
			delay(100);
		}
		digitalWrite(EPAPER_FRAME_PIN_TFT_ENABLE, HIGH);
		digitalWrite(EPAPER_FRAME_PIN_ITE_ENABLE, HIGH);
		delay(10);
		s_spi.begin(EPAPER_FRAME_PIN_SCK, EPAPER_FRAME_PIN_MISO, EPAPER_FRAME_PIN_MOSI, -1);

#if EPAPER_FRAME_PIN_RES >= 0
		pinMode(EPAPER_FRAME_PIN_RES, OUTPUT);
		digitalWrite(EPAPER_FRAME_PIN_RES, HIGH);
		delay(10);
		digitalWrite(EPAPER_FRAME_PIN_RES, LOW);
		delay(10);
		digitalWrite(EPAPER_FRAME_PIN_RES, HIGH);
		delay(10);
#endif
		...
		delay(100);
		if (!read_dev_info() || !set_vcom(EPAPER_FRAME_VCOM_MV) ||
...
void log_wake_diagnostics() {
		...
		LOGW("Epaper", "HRDY wake summary: timeouts=%lu phase=%s wait=%lums recovery=%lu/%lu/%lu",
				...
				epaper_frame_driver_hrdy_phase_name(s_diagnostics.last_hrdy_timeout_phase),
...
// this, a draw after epaper_frame_driver_sleep() would issue commands to a controller
...
bool upload_frame() {
		...
		args[1] = 0;
		args[2] = 0;
		args[3] = EPAPER_FRAME_PANEL_W;
		args[4] = EPAPER_FRAME_PANEL_H;
		...
		const uint16_t WB = EPAPER_FRAME_PANEL_W / 2;  // packed 4 bpp bytes per row
		...
		static uint8_t rowbuf[EPAPER_FRAME_PANEL_W / 2];  // packed 4 bpp pixels for one row
		for (uint16_t row = 0; row < EPAPER_FRAME_PANEL_H; row++) {
...
bool refresh_gc16() {
		...
		if (!write_cmd16(CMD_DPY_AREA) ||
				...
				!write_data16(EPAPER_FRAME_PANEL_W) || !write_data16(EPAPER_FRAME_PANEL_H) ||
...
bool load_g16p_to_canvas(const uint8_t* data, size_t len, bool strict) {
		...
		if (version != G16P_VERSION || flags != 0 ||
				width != EPAPER_FRAME_PANEL_W || height != EPAPER_FRAME_PANEL_H ||
		...
		if (strict && epaper_frame_validate_g16p(data, len, EPAPER_FRAME_PANEL_W, EPAPER_FRAME_PANEL_H) !=
...
bool inflate_g16z(const uint8_t* data, size_t len, uint8_t** out_buf, size_t* out_len,
		...
		*out_len = 0;
		...
		const EpaperMediaValidation completion = epaper_frame_validate_g16z_completion(
...
int jpeg_draw_cb(JPEGDRAW* pDraw) {
		...
		for (int row = 0; row < pDraw->iHeight; row++) {
				...
				if (y < 0 || y >= EPAPER_FRAME_PANEL_H) continue;
				const uint8_t* src = px + (uint32_t)row * pDraw->iWidth;
				if (s_gray16) {
						int16_t* dst = s_gray16 + (uint32_t)y * EPAPER_FRAME_PANEL_W;
						for (int col = 0; col < cols; col++) {
								const int x = pDraw->x + col;
								if (x < 0 || x >= EPAPER_FRAME_PANEL_W) continue;
								dst[x] = src[col];
						}
				} else {
						for (int col = 0; col < cols; col++) {
...
void gray16_dither_to_canvas() {
		const int W = EPAPER_FRAME_PANEL_W;
		const int H = EPAPER_FRAME_PANEL_H;
...
// epaper_frame_sd_cache module (see begin()) and invoked by it after every SD.end(),
...
void restore_panel_bus_cb() {
		s_spi.begin(EPAPER_FRAME_PIN_SCK, EPAPER_FRAME_PIN_MISO, EPAPER_FRAME_PIN_MOSI, -1);
		pinMode(EPAPER_FRAME_PIN_CS, OUTPUT);
		digitalWrite(EPAPER_FRAME_PIN_CS, HIGH);
}
#endif // EPAPER_FRAME_SD_CS_PIN
...
bool epaper_frame_driver_begin() {
		if (s_began) return true;

		if (!s_canvas) {
				s_canvas = new Gray16Canvas(EPAPER_FRAME_PANEL_W, EPAPER_FRAME_PANEL_H);
		...
		pinMode(EPAPER_FRAME_PIN_CS, OUTPUT);
		digitalWrite(EPAPER_FRAME_PIN_CS, HIGH);
		pinMode(EPAPER_FRAME_PIN_BUSY, INPUT);
		s_diagnostics.initial_hrdy_pin_state = digitalRead(EPAPER_FRAME_PIN_BUSY) == HIGH ? 1 : 0;
		s_diagnostics.initial_hrdy_pin_state_valid = true;
		pinMode(EPAPER_FRAME_PIN_TFT_ENABLE, OUTPUT);
		digitalWrite(EPAPER_FRAME_PIN_TFT_ENABLE, HIGH);
		pinMode(EPAPER_FRAME_PIN_ITE_ENABLE, OUTPUT);
		digitalWrite(EPAPER_FRAME_PIN_ITE_ENABLE, HIGH);
		...
		pinMode(EPAPER_FRAME_SD_CS_PIN, OUTPUT);
		digitalWrite(EPAPER_FRAME_SD_CS_PIN, HIGH);
#ifdef EPAPER_FRAME_SD_EN_PIN
		pinMode(EPAPER_FRAME_SD_EN_PIN, OUTPUT);
		digitalWrite(EPAPER_FRAME_SD_EN_PIN, HIGH);
#endif
#endif
		...
		const EpaperSdCacheConfig sd_cfg = {
				&s_spi,
				EPAPER_FRAME_SD_CS_PIN,
#ifdef EPAPER_FRAME_SD_EN_PIN
				EPAPER_FRAME_SD_EN_PIN,
#else
				-1,
#endif
#ifdef EPAPER_FRAME_SD_DET_PIN
				EPAPER_FRAME_SD_DET_PIN,
#else
				-1,
#endif
				&prepare_sd_bus_cb,
		...
		epaper_frame_sd_cache_init(sd_cfg);
#endif
...
void begin_worker(void* arg) {
		...
		ctx->ok = epaper_frame_driver_begin();
		xTaskNotifyGive(ctx->caller);
...
void epaper_frame_driver_begin_async() {
		...
		if (!s_begin_spawned) {
				...
				s_begin_result = epaper_frame_driver_begin();
		}
}

bool epaper_frame_driver_begin_join() {
...
bool epaper_frame_driver_battery_ready_before_begin() { return true; }

void epaper_frame_driver_set_rotation(uint8_t rotation) {
...
static bool draw_blob(const char* source, uint8_t* data, size_t len, bool strict) {
		...
		if (strict && epaper_frame_validate_jpeg(data, len, EPAPER_FRAME_PANEL_W, EPAPER_FRAME_PANEL_H) !=
				EpaperMediaValidation::Valid) {
		...
		s_gray16 = (int16_t*)heap_caps_malloc(
				(size_t)EPAPER_FRAME_PANEL_W * EPAPER_FRAME_PANEL_H * sizeof(int16_t),
				MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
		if (!s_gray16) {
...
bool epaper_frame_driver_draw_url(const char* url) {
		...
		const bool downloaded = epaper_frame_http_download(url, &data, &len);
...
bool epaper_frame_driver_prepare_service_blob(const uint8_t* data, size_t len, const char* media_type,
		...
		*prepared_len = 0;
		...
		if (media_type && strcmp(media_type, "application/vnd.photoframe.g16p") == 0) {
				return epaper_frame_validate_g16p(data, len, EPAPER_FRAME_PANEL_W, EPAPER_FRAME_PANEL_H) ==
						EpaperMediaValidation::Valid;
		}
		if (media_type && strcmp(media_type, "image/jpeg") == 0) {
				return epaper_frame_validate_jpeg(data, len, EPAPER_FRAME_PANEL_W, EPAPER_FRAME_PANEL_H) ==
						EpaperMediaValidation::Valid;
		}
		if ((media_type && strcmp(media_type, "application/vnd.photoframe.g16z") == 0) ||
				...
				const bool ok = inflate_g16z(data, len, &inflated, &inflated_len, true) &&
						epaper_frame_validate_g16p(inflated, inflated_len, EPAPER_FRAME_PANEL_W, EPAPER_FRAME_PANEL_H) ==
						EpaperMediaValidation::Valid;
				if (!ok) {
						if (inflated) heap_caps_free(inflated);
						return false;
...
bool epaper_frame_driver_draw_service_blob(const uint8_t* data, size_t len, const char* media_type,
...
bool epaper_frame_driver_display() {
...
const EpaperDriverDiagnostics& epaper_frame_driver_diagnostics() {
		return s_diagnostics;
}

const char* epaper_frame_driver_hrdy_phase_name(EpaperHrdyPhase phase) {
		switch (phase) {
...
uint16_t epaper_frame_driver_battery_mv() {
#if EPAPER_FRAME_BATTERY_ADC_PIN >= 0
		// Gate the divider on (E1003 routes battery sense through an enable pin)
		// and give the rail a brief settle before sampling.
#if EPAPER_FRAME_BATTERY_ENABLE_PIN >= 0
		pinMode(EPAPER_FRAME_BATTERY_ENABLE_PIN, OUTPUT);
		digitalWrite(EPAPER_FRAME_BATTERY_ENABLE_PIN, HIGH);
		delay(5);
#else
		delay(10);
#endif
		const uint32_t raw_mv = analogReadMilliVolts(EPAPER_FRAME_BATTERY_ADC_PIN);
#if EPAPER_FRAME_BATTERY_ENABLE_PIN >= 0
		digitalWrite(EPAPER_FRAME_BATTERY_ENABLE_PIN, LOW);  // power the divider back down
#endif
		return (uint16_t)((float)raw_mv * EPAPER_FRAME_BATTERY_DIVIDER);
#else
		return 0;
#endif
}
...
// ---------------------------------------------------------------------------
// SD image cache HAL surface (see epaper_frame_driver.h). Thin pass-throughs to the
// shared epaper_frame_sd_cache module. Defined only when this board has a shared-bus
// microSD slot (EPAPER_FRAME_SD_CS_PIN); otherwise the header inline no-ops apply.
// ---------------------------------------------------------------------------
#ifdef EPAPER_FRAME_SD_CS_PIN
void epaper_frame_driver_set_sd_cache_enabled(bool enabled) {
		epaper_frame_sd_cache_set_enabled(enabled);
}

void epaper_frame_driver_cache_flush() {
		epaper_frame_sd_cache_flush();
}

bool epaper_frame_driver_sd_cache_clear() {
		return epaper_frame_sd_cache_clear();
}
#endif // EPAPER_FRAME_SD_CS_PIN
...
void epaper_frame_driver_clear() {
		if (!s_began || !s_canvas) return;
		s_canvas->fillScreen(15);  // white
}

void epaper_frame_driver_fill_rect(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t color) {
		if (!s_began || !s_canvas) return;
		s_canvas->fillRect(x, y, w, h, hal_to_gray4(color));
}

void epaper_frame_driver_draw_rect(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t color) {
		if (!s_began || !s_canvas) return;
		s_canvas->drawRect(x, y, w, h, hal_to_gray4(color));
}

void epaper_frame_driver_draw_round_rect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r, uint8_t color) {
		if (!s_began || !s_canvas) return;
		s_canvas->drawRoundRect(x, y, w, h, r, hal_to_gray4(color));
}

void epaper_frame_driver_fill_round_rect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r, uint8_t color) {
		if (!s_began || !s_canvas) return;
		s_canvas->fillRoundRect(x, y, w, h, r, hal_to_gray4(color));
}

void epaper_frame_driver_set_font(uint8_t font_id) {
		if (!s_began || !s_canvas) return;
		if (font_id > EPAPER_FRAME_FONT_LARGE) font_id = EPAPER_FRAME_FONT_MEDIUM;
		s_canvas->setFont(s_font_table[font_id]);
}

void epaper_frame_driver_set_text_color(uint8_t color) {
		if (!s_began || !s_canvas) return;
		s_canvas->setTextColor(hal_to_gray4(color));
}

void epaper_frame_driver_set_cursor(int16_t x, int16_t y) {
		if (!s_began || !s_canvas) return;
		s_canvas->setCursor(x, y);
}

void epaper_frame_driver_print(const char* text) {
		if (!s_began || !s_canvas || !text) return;
		s_canvas->print(text);
}

void epaper_frame_driver_get_text_bounds(const char* text, int16_t x, int16_t y,
...
int16_t epaper_frame_driver_width() {
		if (!s_began || !s_canvas) return 0;
		return s_canvas->width();
}

int16_t epaper_frame_driver_height() {
		if (!s_began || !s_canvas) return 0;
...
float epaper_frame_driver_read_vcom() {
		return NAN;
}

bool epaper_frame_driver_write_vcom(float /*vcom*/) {
		return false;
}

void epaper_frame_driver_show_vcom_test_pattern(float /*preview_vcom*/) {
		// No-op: IT8951 VCOM is not user-calibratable through this portal page.
}

#endif // IS_EPAPER_FRAME && defined(BOARD_RETERMINAL_E1003_FRAME)
