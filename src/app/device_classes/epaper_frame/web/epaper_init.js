...
if (typeof window.registerConfigFields === 'function') {
    window.registerConfigFields([
        ...
        'epaper_frame_wake_mqtt_target_ms', 'epaper_frame_wake_mqtt_budget_ms',
...
window.init_epaper_image_fragment = function () {
    ...
    var sourceMode = document.getElementById('epaper_frame_source_mode');
    ...
    function loadCarouselAndSchedule() {
        fetch('/api/config')
            ...
            .then(function (cfg) {
                ...
                if (sourceMode) {
                    sourceMode.value = serviceSupported && cfg.epaper_frame_source_mode === 'service'
                ...
                setNamedValue('epaper_frame_wake_fetch_target_ms', cfg.epaper_frame_wake_fetch_target_ms || 2000);
                setNamedValue('epaper_frame_wake_fetch_budget_ms', cfg.epaper_frame_wake_fetch_budget_ms || 3500);
                setNamedValue('epaper_frame_wake_mqtt_target_ms', cfg.epaper_frame_wake_mqtt_target_ms || 750);
                setNamedValue('epaper_frame_wake_mqtt_budget_ms', cfg.epaper_frame_wake_mqtt_budget_ms || 1500);
                setNamedValue('epaper_frame_wake_cutoff_retry_seconds', cfg.epaper_frame_wake_cutoff_retry_seconds || 0);
                updateWriteOnlySecretField('epaper_frame_service_token', 'epaper-service-token-status',
                    serviceTokenSet, 'Not configured.', 'token');
                updateSourceSections();

                var scheduleMask = (cfg.epaper_frame_schedule_hours == null)
                    ? 0x00FFFFFF
                    : (parseInt(cfg.epaper_frame_schedule_hours, 10) & 0x00FFFFFF);
                if (isNaN(scheduleMask)) scheduleMask = 0x00FFFFFF;
                writeHoursMask(scheduleMask >>> 0);
                if (tzSelect && cfg.epaper_frame_schedule_tz_offset != null) {
                    tzSelect.value = String(cfg.epaper_frame_schedule_tz_offset);
                }

                setNamedValue('epaper_frame_crc32_enabled', !!cfg.epaper_frame_crc32_enabled);
    ...
    async function saveImageConfig() {
        ...
        config.epaper_frame_schedule_hours = readHoursMask();
        ...
        if (usingService) {
            ...
            if (!serviceTokenSet && (!config.epaper_frame_service_token || !String(config.epaper_frame_service_token).trim())) {
                showMessage('Bearer token is required.', 'error');
                return;
            }
            var serviceInterval = parseInt(config.epaper_frame_service_interval_seconds || '0', 10);
            if (isNaN(serviceInterval) || serviceInterval <= 0) {
                showMessage('Service refresh interval must be greater than 0 seconds.', 'error');
                return;
            }
            config.epaper_frame_service_interval_seconds = serviceInterval;
        } else {
            ...
            config.epaper_frame_carousel = carouselPayload.value;
        }
        ...
        try {
            ...
            if (result && result.success) {
                if (usingService && config.epaper_frame_service_token) {
...
window.init_epaper_overlay_fragment = function () {
    ...
    var overlayHidden = document.getElementById('epaper_frame_overlay_items');
