...
void handlePostScaleTare(AsyncWebServerRequest* request) {
    ...
    snprintf(coffee_scale_config.scale_offset, COFFEE_SCALE_CAL_MAX_LEN, "%ld", scale_get_offset());
...
void handlePostScaleCalibrate(AsyncWebServerRequest* request, uint8_t* data, size_t len, size_t index, size_t total) {
    ...
    snprintf(coffee_scale_config.scale_cal_factor, COFFEE_SCALE_CAL_MAX_LEN, "%.4f", scale_get_calibration_factor());
    snprintf(coffee_scale_config.scale_offset, COFFEE_SCALE_CAL_MAX_LEN, "%ld", scale_get_offset());
    config_manager_save(&device_config);
...
REGISTER_ROUTES(web_portal_scale_register_routes);
