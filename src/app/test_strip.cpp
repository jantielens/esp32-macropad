...
static float current_step_value() {
    return STEP_TABLE[s.step_idx].value;
}

static const char* current_step_label() {
    return STEP_TABLE[s.step_idx].label;
}
...
static void recalculate_segments() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        float cum = snap_tenth(s.base_time_s * powf(2.0f, offset));
        if (cum < 0.1f) cum = 0.1f;

        s.segments[i].cumulative_s = cum;
        s.segments[i].offset_stops = offset;

        if (i == n - 1) {
            // Last segment (least light): incremental == cumulative
            s.segments[i].incremental_s = cum;
        } else {
            // Incremental = difference to next (smaller) cumulative
            float next_cum = snap_tenth(s.base_time_s * powf(2.0f, (center - (i + 1)) * step));
            if (next_cum < 0.1f) next_cum = 0.1f;
            float inc = cum - next_cum;
            if (inc < 0.1f) inc = 0.1f;
            s.segments[i].incremental_s = snap_tenth(inc);
        }
    }
}
...
static uint32_t phase_elapsed_ms() {
    if (s.phase == STRIP_IDLE) return 0;
    return millis() - s.phase_start_ms;
}

static uint32_t phase_remaining_ms() {
    if (s.phase == STRIP_IDLE) return 0;
    uint32_t elapsed = phase_elapsed_ms();
    return (elapsed >= s.phase_duration_ms) ? 0 : (s.phase_duration_ms - elapsed);
}
...
static float estimate_total_time_s() {
    ...
    float sum = 0;
    for (int i = 0; i < n; i++) {
        sum += s.segments[i].incremental_s;
    }
...
static void enter_phase(StripPhase phase, uint32_t duration_ms) {
    s.phase = phase;
    s.phase_start_ms = millis();
    s.phase_duration_ms = duration_ms;
    s.last_tick_ms = 0;
    s.pre_beep_fired = false;
}

static void start_sequence() {
    if (s.phase != STRIP_IDLE) return;

    recalculate_segments();
    s.current_segment = 0;

    LOGI(TAG, "Start: %d segs, base %.1fs, step %s stops, countdown %ds, pause %ds",
         s.segment_count, s.base_time_s, current_step_label(), s.countdown_s, s.pause_s);

    // Log segment table
    for (int i = 0; i < s.segment_count; i++) {
        LOGI(TAG, "  Seg %d: cum=%.1fs inc=%.1fs offset=%+.3f stops",
             i + 1, s.segments[i].cumulative_s, s.segments[i].incremental_s,
             s.segments[i].offset_stops);
    }

    enter_phase(STRIP_INITIAL_COUNTDOWN, s.countdown_s * 1000U);
}

static void start_exposing() {
    s.current_segment++;
    int idx = s.current_segment - 1;
    ...
    LOGI(TAG, "Exposing seg %d/%d: %.1fs (cum %.1fs)",
         ...
         s.segments[idx].incremental_s, s.segments[idx].cumulative_s);
}
...
static void sequence_complete() {
    ...
    LOGI(TAG, "Sequence complete: %d segments, range %.1f-%.1fs",
         s.segment_count,
         s.segments[s.segment_count - 1].cumulative_s,
         s.segments[0].cumulative_s);
...
static void cmd_add_base(float delta) {
    cmd_set_base(s.base_time_s + delta);
}

static void cmd_step_up() {
    if (s.step_idx < STEP_TABLE_SIZE - 1) {
        s.step_idx++;
        recalculate_segments();
...
static void cmd_add_segments(int delta) {
    // Step by 2 to stay on odd numbers
    if (delta > 0 && delta < 2) delta = 2;
    if (delta < 0 && delta > -2) delta = -2;
    int n = clamp_odd_segments(s.segment_count + delta);
    s.segment_count = n;
    recalculate_segments();
...
static void cmd_set_segments(int n) {
    s.segment_count = clamp_odd_segments(n);
    recalculate_segments();
    LOGI(TAG, "Segments: %d", s.segment_count);
}
...
static bool strip_resolve(const char* params, char* out, size_t out_len) {
    ...
    if (strcmp(params, "segment") == 0) {
        snprintf(out, out_len, "%d", s.current_segment);
        return true;
    }

    // segments (total count)
    if (strcmp(params, "segments") == 0) {
        snprintf(out, out_len, "%d", s.segment_count);
        return true;
    }
    ...
    if (strncmp(params, "seg_time:", 9) == 0) {
        int n = atoi(params + 9);
        if (n >= 1 && n <= s.segment_count) {
            snprintf(out, out_len, "%.1f", s.segments[n - 1].cumulative_s);
            return true;
        }
    ...
    if (strncmp(params, "seg_offset:", 11) == 0) {
        ...
        if (n >= 1 && n <= s.segment_count) {
            snprintf(out, out_len, "%+.1f", s.segments[n - 1].offset_stops);
            return true;
        }
        snprintf(out, out_len, "---");
        return true;
    ...
    if (strncmp(params, "seg_inc:", 8) == 0) {
        int n = atoi(params + 8);
        if (n >= 1 && n <= s.segment_count) {
            snprintf(out, out_len, "%.1f", s.segments[n - 1].incremental_s);
            return true;
        }
        snprintf(out, out_len, "---");
        return true;
    ...
    if (strcmp(params, "seg_inc") == 0) {
        if (s.current_segment >= 1 && s.current_segment <= s.segment_count) {
            snprintf(out, out_len, "%.1f", s.segments[s.current_segment - 1].incremental_s);
        } else {
            snprintf(out, out_len, "---");
        }
        return true;
    ...
    if (strcmp(params, "base_time") == 0) {
        snprintf(out, out_len, "%.1f", s.base_time_s);
        return true;
    }
    ...
    if (strcmp(params, "progress") == 0) {
        snprintf(out, out_len, "%d/%d", s.current_segment, s.segment_count);
        return true;
    }
    ...
    if (strcmp(params, "total_time") == 0) {
        ...
        if (format) {
            format_ms((uint32_t)(total * 1000.0f), format, out, out_len);
        } else {
    ...
    if (strcmp(params, "countdown") == 0) {
        snprintf(out, out_len, "%d", s.countdown_s);
        return true;
    }

    // pause (inter-segment pause setting)
    if (strcmp(params, "pause") == 0) {
        snprintf(out, out_len, "%d", s.pause_s);
        return true;
    }

    // tick (exposure tick setting)
    if (strcmp(params, "tick") == 0) {
        snprintf(out, out_len, "%s", s.exposure_tick ? "on" : "off");
        return true;
    }
    ...
    if (strcmp(params, "table") == 0) {
        ...
        for (int i = 0; i < n && pos < (int)out_len - 80; i++) {
            ...
            bool active = (s.phase == STRIP_EXPOSING && i == s.current_segment - 1) ||
                          (s.phase == STRIP_BETWEEN_SEGMENTS && i == s.current_segment);
            if (active) {
                pos += snprintf(out + pos, out_len - pos,
                    "{\"seg\":\"%d\",\"dur\":\"%.1fs\",\"tot\":\"%.1fs\",\"_bg\":\"#401010\"}",
                    i + 1, s.segments[i].incremental_s, s.segments[i].cumulative_s);
            } else {
                pos += snprintf(out + pos, out_len - pos,
...
void test_strip_dispatch(const char* command) {
    ...
    if (s.phase != STRIP_IDLE) {
        LOGW(TAG, "Config rejected (sequence active): '%s'", command);
        return;
    }

    if (strncmp(command, "set_base:", 9) == 0) {
        cmd_set_base(strtof(command + 9, NULL));
        return;
    }
    if (strncmp(command, "add_base:", 9) == 0) {
        cmd_add_base(strtof(command + 9, NULL));
        return;
    }
    ...
    if (strncmp(command, "add_segments:", 13) == 0) {
        cmd_add_segments(atoi(command + 13));
        return;
    }
    if (strncmp(command, "set_segments:", 13) == 0) {
        cmd_set_segments(atoi(command + 13));
        return;
    }
    if (strncmp(command, "set_countdown:", 14) == 0) {
        cmd_set_countdown(atoi(command + 14));
        return;
    }
    if (strncmp(command, "set_pause:", 10) == 0) {
        cmd_set_pause(atoi(command + 10));
        return;
    }
    if (strncmp(command, "set_tick:", 9) == 0) {
        cmd_set_tick(strcmp(command + 9, "on") == 0);
        return;
    }
...
void test_strip_tick() {
    ...
    uint32_t now = millis();
    uint32_t elapsed = now - s.phase_start_ms;

    switch (s.phase) {

    case STRIP_INITIAL_COUNTDOWN: {
#if HAS_AUDIO
        uint32_t remaining = (elapsed < s.phase_duration_ms)
                             ? s.phase_duration_ms - elapsed : 0;
        // Per-second ticks during the early portion
        if (remaining > PRE_EXPOSE_BEEP_LEAD_MS) {
            uint32_t tick_sec = elapsed / 1000;
            uint32_t last_sec = (s.last_tick_ms > 0)
                                ? (s.last_tick_ms - s.phase_start_ms) / 1000 : UINT32_MAX;
        ...
        if (!s.pre_beep_fired && remaining <= PRE_EXPOSE_BEEP_LEAD_MS) {
            audio_beep(AUDIO_PRE_EXPOSE_BEEP, 0);
            s.pre_beep_fired = true;
        }
#endif
        if (elapsed >= s.phase_duration_ms) {
            start_exposing();
        }
        break;
    }
    ...
    case STRIP_EXPOSING: {
        ...
        if (s.exposure_tick && s.phase_duration_ms > 2000) {
            uint32_t tick_sec = elapsed / 1000;
            uint32_t last_sec = (s.last_tick_ms > 0)
                                ? (s.last_tick_ms - s.phase_start_ms) / 1000 : UINT32_MAX;
        ...
        if (elapsed >= s.phase_duration_ms) {
            // Segment complete
            if (s.current_segment >= s.segment_count) {
                // Last segment
                sequence_complete();
            } else {
