...
void action_parse(const JsonObject& a, ButtonAction& act) {
    ...
    strlcpy(act.volume_value, a["volume_value"] | "", CONFIG_VALUE_MAX_LEN);
    strlcpy(act.brightness_mode, a["brightness_mode"] | "", CONFIG_VOLUME_MODE_MAX_LEN);
    strlcpy(act.brightness_value, a["brightness_value"] | "", CONFIG_VALUE_MAX_LEN);
    // Timer fields
    act.timer_id = (uint8_t)(a["timer_id"] | 0);
    strlcpy(act.timer_command, a["timer_command"] | "", CONFIG_TIMER_CMD_MAX_LEN);
    strlcpy(act.timer_value, a["timer_value"] | "", CONFIG_VALUE_MAX_LEN);
...
void action_to_json(const ButtonAction& act, JsonObject obj) {
    ...
    // Timer fields
    if (act.timer_id > 0)         obj["timer_id"]          = act.timer_id;
    if (act.timer_command[0])     obj["timer_command"]     = act.timer_command;
    if (act.timer_value[0])       obj["timer_value"]       = act.timer_value;
