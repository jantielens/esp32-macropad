...
void action_parse(const JsonObject& a, ButtonAction& act) {
    ...
    // Expose: "expose_command" from web → reuse mqtt_payload for storage
    ...
    // Mem: "mem_command" from web → reuse mqtt_payload for storage
    if (strcmp(act.type, ACTION_TYPE_MEM) == 0 && a.containsKey("mem_command")) {
        strlcpy(act.mqtt_payload, a["mem_command"] | "", CONFIG_MQTT_PAYLOAD_MAX_LEN);
    }
...
void action_to_json(const ButtonAction& act, JsonObject obj) {
    ...
    if (act.mqtt_payload[0]) {
        if (strcmp(act.type, ACTION_TYPE_TIMER) == 0) {
            ...
            obj["mem_command"] = act.mqtt_payload;
        } else {
