...
cpp_epaper="$(  extract_full_name EPAPER_FRAME)"
...
for pair in "macropad=$cpp_macropad" "epaper_frame=$cpp_epaper" "headless=$cpp_headless" "shutter_tester=$cpp_shutter" "coffee_scale=$cpp_coffee" "darkroom_timer=$cpp_darkroom" "voice_assistant=$cpp_voice"; do
    cls="${pair%%=*}"
    val="${pair#*=}"
    if [[ -z "$val" ]]; then
...
source "$CFG"
sh_epaper="$(  device_class_brand_prefix epaper_frame)"
...
check epaper_frame   "$cpp_epaper"   "$sh_epaper"
