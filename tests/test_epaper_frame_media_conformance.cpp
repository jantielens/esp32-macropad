...
EpaperMediaValidation validate_g16z(const std::vector<uint8_t>& data) {
		...
		return epaper_frame_validate_g16p(output.data(), output.size(), kPanelWidth, kPanelHeight);
}
...
int main() {
		...
		assert(epaper_frame_validate_jpeg(wrong_dimensions_jpeg.data(), wrong_dimensions_jpeg.size(),
				kPanelWidth, kPanelHeight) == EpaperMediaValidation::WrongGeometry);
