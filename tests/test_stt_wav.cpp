...
int main() {
    uint8_t header[STT_WAV_HEADER_BYTES] = {};
    stt_wav_build_header(header, 960000, 16000, 1, 16);

    check(std::memcmp(header, "RIFF", 4) == 0, "RIFF signature");
    check(read_u32_le(header + 4) == 960036, "RIFF payload size");
    check(std::memcmp(header + 8, "WAVEfmt ", 8) == 0, "WAV/fmt identifiers");
    check(read_u32_le(header + 16) == 16, "PCM format chunk size");
    ...
    std::puts("STT WAV checks passed");
    return 0;
}