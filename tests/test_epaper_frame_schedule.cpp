...
#include "device_classes/epaper_frame/epaper_frame_schedule.h"
...
TEST_F(EpaperScheduleTest, FailOpenOnStaleClock) {
		...
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(0x0, 0, EPOCH_1999_STALE));  // all disabled, but clock stale
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(0x0, 0, EPOCH_1999_STALE - 1000));
}
...
TEST_F(EpaperScheduleTest, HourZeroEnabledAtMidnight) {
		...
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(bitmask, 0, EPOCH_2000_01_01_00_00));
}
...
TEST_F(EpaperScheduleTest, HourTwentyThreeEnabledAt11pm) {
		...
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(bitmask, 0, epoch_23_00_utc));
}
...
TEST_F(EpaperScheduleTest, HourDisabledReturnsFalse) {
		...
		EXPECT_FALSE(epaper_frame_schedule_should_refresh(bitmask, 0, EPOCH_2000_01_01_12_00));
}
...
TEST_F(EpaperScheduleTest, TimezoneOffsetPositive) {
		...
		EXPECT_FALSE(epaper_frame_schedule_should_refresh(bitmask, 5, epoch_12_00_utc));

		// But hour 17 enabled should work
		bitmask = (1U << 17);
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(bitmask, 5, epoch_12_00_utc));
}
...
TEST_F(EpaperScheduleTest, TimezoneWrapAroundNegative) {
		...
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(bitmask, -5, EPOCH_2000_01_01_00_00));
}
...
TEST_F(EpaperScheduleTest, TimezoneWrapAroundPositive) {
		...
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(bitmask, 5, epoch_23_00_utc));
}

// Test: all hours disabled (bitmask=0) returns false
TEST_F(EpaperScheduleTest, AllHoursDisabled) {
		EXPECT_FALSE(epaper_frame_schedule_should_refresh(0x0, 0, EPOCH_2000_01_01_12_00));
}

// Test: all hours enabled (bitmask=0xFFFFFF)
TEST_F(EpaperScheduleTest, AllHoursEnabled) {
		EXPECT_TRUE(epaper_frame_schedule_should_refresh(0xFFFFFF, 0, EPOCH_2000_01_01_12_00));
}

// Test: seconds_to_next when all hours disabled (mask=0)
TEST_F(EpaperScheduleTest, SecondsToNextAllDisabledReturn3600) {
		uint32_t secs = epaper_frame_schedule_seconds_to_next(0x0, 0, EPOCH_2000_01_01_12_00);
		EXPECT_EQ(secs, 3600);
}
...
TEST_F(EpaperScheduleTest, SecondsToNextFindNextEnabledHour) {
		...
		uint32_t secs = epaper_frame_schedule_seconds_to_next(bitmask, 0, epoch_10_00);
...
TEST_F(EpaperScheduleTest, SecondsToNextWrapsToNextDay) {
		...
		uint32_t secs = epaper_frame_schedule_seconds_to_next(bitmask, 0, epoch_23_00);
		// Should be approximately 2 hours = 7200 seconds
...
TEST_F(EpaperScheduleTest, SecondsToNextWithTimezoneOffset) {
		...
		uint32_t secs = epaper_frame_schedule_seconds_to_next(bitmask, 5, epoch_12_00_utc);
		EXPECT_GE(secs, 7200 - 60);
		EXPECT_LE(secs, 7200 + 60);
}
