...
A struck clock brick, including either colon brick, disappears, then returns in
its original location after the configured respawn delay.
...
| `respawn_ms` | `1000` | Delay before a hit clock brick returns. The supported range is `0` to `10000` milliseconds. |
| `paddle_accuracy` | `96` | Autonomous paddle tracking accuracy from `0` to `100`. The default provides near-perfect play. |
...
only that brick to the damaged region. The game has no network or worker
activity.
