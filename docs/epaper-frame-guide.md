...
The E-Paper Frame device class is the low-power, non-interactive branch of ESP32 Macropad.
...
The initial defaults live in `src/app/device_classes/epaper_frame/epaper_wake_budget.h`.
...
The E-Paper Frame boards are materially different from the interactive boards in this repository.

* `HAS_EPAPER_PANEL` is enabled
* `HAS_DISPLAY` is disabled
* `HAS_TOUCH` is disabled
* `HAS_BUTTON` is disabled
* `HAS_EPAPER_FRAME_WAKE_BUTTON` is enabled
...
* **Position** &mdash; one of the four panel corners, set via the E-Paper Frame portal page.
...
The portal exposes three actions under the VCOM card on the E-Paper Frame page:
...
On E-Paper Frame boards, the web portal uses a dedicated **E-Paper Frame** category as the primary landing area. The category contains five pages:
...
The operating mode is not exposed as a separate page on E-Paper Frame boards. The Image Sources and Status Overlay pages each write the hidden `operating_mode=duty_cycle_epaper_frame` field on save so the board cannot drift into an unrelated transport mode through normal portal use.
...
The E-Paper Frame device class is intentionally narrow in this first version.
