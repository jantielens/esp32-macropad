...
```mermaid
graph TD
    A["Arduino_SWSPI<br/>(9-bit SPI, ST7701 init commands)"]
    B["Arduino_ESP32RGBPanel<br/>(RGB parallel bus, timing, bounce buffer)"]
    C["Arduino_RGB_Display<br/>(framebuffer management, auto_flush)"]
    A --> B --> C
```
