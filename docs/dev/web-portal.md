...
Display builds expose a user-managed recipe catalog at `/config/recipes.json`.
The file is absent until the user first saves it; the read endpoint then returns
an empty valid catalog envelope. Catalog writes are limited to 64 KiB, require
valid JSON, and publish atomically on both LittleFS and SD storage.

| Method | Path | Description |
| --- | --- | --- |
| `GET` | `/api/recipes/catalog` | Returns the persisted catalog or an empty valid catalog when it has not been saved |
| `POST` | `/api/recipes/catalog` | Validates and atomically saves a complete catalog document |
