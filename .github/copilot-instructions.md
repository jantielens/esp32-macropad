...
- **E-Paper BLE Bridge** (`IS_EPAPER_BLE_BRIDGE`): Headless ESP32-S3 device class that polls up to two e-paper assignment records, broadcasts fixed 22-byte assignment packets through non-connectable legacy BLE advertising, passively receives authenticated ACKs, and forwards them to the existing assignment API. Fixed-size state supports the no-PSRAM target. HTTP runs on a worker task, BLE rotation runs on a radio task, and `on_loop` remains non-blocking. Per-frame config is stored in NVS and exposed through `/api/config` with API keys redacted.
...
| esp32s3-ble-bridge | ESP32-S3 | Headless BLE/WiFi bridge | 4MB, no PSRAM |
