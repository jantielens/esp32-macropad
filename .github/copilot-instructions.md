...
- **Main-loop bridge** (`main_loop_bridge.{h,cpp}`): Neutral web/async-task → main-loop deferral (`loop_bridge_dispatch()` + `loop_bridge_drain()`, single in-flight slot + binary semaphore, bounded wait). Independent of `HAS_MCP` so any web handler can defer LVGL/blocking work; `web_mcp` aliases its old `mcp_control_*` names onto it. Drained each `loop()` from `web_portal_handle()`.
...
- **E-Paper BLE Bridge** (`IS_EPAPER_BLE_BRIDGE`): Headless ESP32-S3 device class that polls up to two e-paper assignment records, broadcasts fixed 22-byte assignment packets through non-connectable legacy BLE advertising, passively receives authenticated ACKs, and forwards them to the existing assignment API. Fixed-size runtime state stays in internal RAM; the bridge uses the shared `tls_allocator` to route mbedTLS allocations to PSRAM with internal-RAM fallback while BLE and HTTPS coexist. HTTP runs on a worker task, BLE rotation runs on a radio task, and `on_loop` remains non-blocking. Frames keep BLE and WiFi mutually exclusive: BLE-only unchanged/cache-hit paths send the authenticated BLE ACK, while a cache miss tears BLE down before WiFi and commits a successful render through the HTTP assignment sync endpoint. Per-frame config is stored in NVS and exposed through `/api/config` with API keys redacted.
...
| esp32s3-ble-bridge | ESP32-S3 | Headless BLE/WiFi bridge | 4MB + 2MB QSPI PSRAM |
